...
| `ui/imlayout.rs` | 管理所有 cell：加载管线、文件夹扫描/打开/导航、键盘事件、标题、网格布局、交互编排 | core, state, imcell | 唯一允许出现线程原语的模块（ADR-0001），线程代码物理隔离在加载/扫描方法组内 |
...
```
用户拖入目录
  → scan_folders() 子线程扫描 → poll_scan() 登记 FolderCell 入格
  → 缩略图批次排队 → 列表/缩略图渲染（imcell::render_folder_cell）
