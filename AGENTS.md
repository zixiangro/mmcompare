...
| `E` / `H` | 切换 EXIF 摘要（首行图片名称 + 相机参数）/ 直方图显示 |
| `1-8` | 旋转对应位置图片（顺时针 90°；导航到下一张后重置为原始方向） |
| `Q` | 按住：两张图时互换显示（对比） |
| `Space` / `B` | 导航：每个文件夹打开 ≤1 张时响应，各文件夹图片同步上一张 / 下一张（单文件夹多图不响应，删到 1 张恢复） |
| `Esc` | 关闭文件夹打开的图片，恢复文件夹视图 |
| 滚轮 | 全局缩放（图片模式下）；文件夹 cell 内为滚动条目 |
| 左键拖拽 | 缩放 >1 时全局平移；局部模式下为框选 |
| 右键拖拽 | 单 cell 平移 |
| `Ctrl` + 右键 | 删除当前 cell；**对比对（2 文件夹各 1 张）禁删图片** |
| `Ctrl` + 左键拖拽 | 重排 |

拖入图片**或文件夹**均可，但两种模式互斥：拖入目录后不再接受文件，反之亦然
（全部删除后可重新选择）。单文件夹可开多张图；多文件夹每文件夹限 1 张
（重复打开自动替换），两个文件夹各开一张即构成对比对，`Space`/`B` 同步索引。
